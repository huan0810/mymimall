          ...
          <div class="item-address">
            ...
            <div class="addr-list clearfix">
              <div class="addr-info" :class="{'checked':checkIndex==index}" @click="checkIndex=index" v-for="(item,index) in list" :key="index">
                ...
                <div class="action">
                  <a href="javascript:;" class="fl" @click="delAddressModal(item)">
                    <!-- svg矢量图 -->
    ...
    <modal title="新增确认" btnType="1" :showModal="showEditModal" @cancel="showEditModal=false" @submit="submitAddress">
      <template v-slot:body>
        <div class="edit-wrap">
          ...
          <div class="item">
            <textarea name="street" v-model="checkedAddress.receiverAddress"></textarea>
          </div>
          <div class="item">
  ...
  methods: {
    ...
    getCartList() {
      ...
      this.axios.get('/carts').then(res => {
        let allList = res.cartProductVoList //获取购物车中所有商品
        this.cartTotalPrice = res.cartTotalPrice //指的是购物车选中的商品总金额
        this.cartList = allList.filter(item => item.productSelected) //过滤出购物车中选中的商品
        // 计算选中商品的件数之和
